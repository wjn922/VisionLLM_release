...
class BertEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        if self.bert_name == "bert-base-uncased":
            config = BertConfig.from_pretrained("checkpoints/%s"%self.bert_name)
            config.gradient_checkpointing = self.cfg.MODEL.LANGUAGE_BACKBONE.USE_CHECKPOINT
            self.model = BertModel.from_pretrained("checkpoints/%s" % self.bert_name, add_pooling_layer=False, config=config)
